...
def matakuliah_detail(request):
    """View untuk melihat detail satu matakuliahwa"""
    dbsession = request.dbsession
...
def matakuliah_add(request):
    """View untuk menambahkan mata kuliah baru"""
    try:
...
def matakuliah_update(request):
    """View untuk mengupdate data mata kuliah"""
    dbsession = request.dbsession
    matakuliah_id = request.matchdict['id']
    ...
    try:
        ...
        if 'kode_mk' in json_data:
            matakuliah.nim = json_data['kode_mk']
        if 'nama_mk' in json_data:
            matakuliah.nama = json_data['nama_mk']
        if 'sks' in json_data:
...
def matakuliah_delete(request):
    """View untuk menghapus datamatakuliah"""
    dbsession = request.dbsession
    matakuliah_id = request.matchdict['id']
    ...
    return {'success': True, 'message': f'Mta Kuliah dengan id {matakuliah_id} berhasil dihapus'}